...
# ════════════════════════════════════════════════════════════════
# OpenClaw on Hugging Face Spaces — startup & supervision script
# ════════════════════════════════════════════════════════════════
...
load_env_bundle() {
  # OPENCLAW_HF_ENV_BUNDLE is a single base64url-encoded JSON object generated
  # by /env-builder. Existing individual env vars win over bundled values.
  local bundle="${OPENCLAW_HF_ENV_BUNDLE:-${ENV_BUNDLE:-}}"
  [ -n "$bundle" ] || return 0
  eval "$(OPENCLAW_HF_ENV_BUNDLE="$bundle" python3 - <<'PYBUNDLE'
import base64, json, os, re, shlex, sys

raw = os.environ.get("OPENCLAW_HF_ENV_BUNDLE", "").strip()
try:
    if raw.startswith("{"):
        data = json.loads(raw)
    else:
    ...
    for key, value in data.items():
        if not re.fullmatch(r"[A-Z_][A-Z0-9_]*", str(key)):
            continue
        if str(key) in {"OPENCLAW_HF_ENV_BUNDLE", "ENV_BUNDLE"}:
            continue
    ...
    print(f"Warning: invalid OPENCLAW_HF_ENV_BUNDLE ignored: {exc}", file=sys.stderr)
PYBUNDLE
)"
}
...
DEVDATA_DATASET_NAME="$(trim_var "${DEVDATA_DATASET_NAME:-openclaw-hf-devdata}")"
...
BACKUP_DATASET="${BACKUP_DATASET_NAME:-openclaw-hf-backup}"
if [ -n "${HF_TOKEN:-}" ]; then
...
CF_PROXY_ENV_FILE="/tmp/openclaw-hf-cloudflare-proxy.env"
...
if [ -n "${HF_TOKEN:-}" ]; then
  echo "Backup    : ${BACKUP_DATASET:-openclaw-hf-backup} (every ${SYNC_INTERVAL:-180}s)"
else
...
if [ -n "${WEBHOOK_URL:-}" ]; then
  WEBHOOK_BODY=$(jq -n \
    --arg model "$LLM_MODEL" \
    '{"event":"restart","status":"success","message":"OpenClaw gateway has started/restarted.","model":$model}')
...
STARTUP_FILE="/home/node/.openclaw/workspace/startup.sh"
_hc_append() {
  if [ "${OPENCLAW_HF_CAPTURE_DISABLE:-0}" = "1" ]; then
    return 0
  fi
  local line="$*"
...
_hc_apt_install() {
  if [ "$(id -u)" -eq 0 ]; then
    ...
    echo "Error: apt install needs root. Rebuild with the latest OpenClaw image or add packages to Dockerfile." >&2
    return 1
  fi
}
apt-get() {
...
# OPENCLAW_HF_STARTUP_STRICT=true to fail the Space startup on any error.
# HC_STARTUP_FAILURES initialized earlier (before Chromium ensure check)
HC_STARTUP_STRICT_NORMALIZED=$(printf '%s' "${OPENCLAW_HF_STARTUP_STRICT:-false}" | tr '[:upper:]' '[:lower:]')
hc_run_startup_command() {
  ...
  set +e
  OPENCLAW_HF_CAPTURE_DISABLE=1 bash -lc "$command_text"
  local rc=$?
  set -e
  if [ "$rc" -eq 0 ]; then
...
hc_run_startup_script() {
  ...
  local script_file
  script_file=$(mktemp "/tmp/openclaw-hf-startup-${source_label//[^A-Za-z0-9_.-]/_}.XXXXXX.sh")
  {
    # Load OpenClaw's install wrappers for env-provided scripts too, so
    ...
    echo 'export OPENCLAW_HF_CAPTURE_DISABLE=1'
...
hc_finish_startup_commands() {
  if [ "$HC_STARTUP_FAILURES" -gt 0 ]; then
    ...
    if [ "$HC_STARTUP_STRICT_NORMALIZED" = "true" ] || [ "$HC_STARTUP_STRICT_NORMALIZED" = "1" ] || [ "$HC_STARTUP_STRICT_NORMALIZED" = "yes" ]; then
      echo "ERROR: OPENCLAW_HF_STARTUP_STRICT=true, stopping startup." >&2
      exit 1
    fi
  fi
  return 0
...
if [ -n "${OPENCLAW_HF_APT_PACKAGES:-}" ]; then
  echo "Installing apt packages from OPENCLAW_HF_APT_PACKAGES..."
  read -r -a HC_APT_PACKAGES <<< "$OPENCLAW_HF_APT_PACKAGES"
  if command -v sudo >/dev/null 2>&1; then
    if sudo apt-get update && sudo apt-get install -y "${HC_APT_PACKAGES[@]}"; then
      echo "OPENCLAW_HF_APT_PACKAGES install complete."
    else
      HC_STARTUP_FAILURES=$((HC_STARTUP_FAILURES + 1))
      echo "ERROR: OPENCLAW_HF_APT_PACKAGES install failed: ${OPENCLAW_HF_APT_PACKAGES}" >&2
    fi
  else
    HC_STARTUP_FAILURES=$((HC_STARTUP_FAILURES + 1))
    echo "ERROR: sudo is unavailable; OPENCLAW_HF_APT_PACKAGES install skipped" >&2
  fi
fi
if [ -n "${OPENCLAW_HF_PIP_PACKAGES:-}" ]; then
  echo "Installing Python packages from OPENCLAW_HF_PIP_PACKAGES..."
  read -r -a HC_PIP_PACKAGES <<< "$OPENCLAW_HF_PIP_PACKAGES"
  if python3 -m pip install --user --break-system-packages "${HC_PIP_PACKAGES[@]}"; then
    echo "OPENCLAW_HF_PIP_PACKAGES install complete."
  else
    HC_STARTUP_FAILURES=$((HC_STARTUP_FAILURES + 1))
    echo "ERROR: OPENCLAW_HF_PIP_PACKAGES install failed: ${OPENCLAW_HF_PIP_PACKAGES}" >&2
  fi
fi
if [ -n "${OPENCLAW_HF_NPM_PACKAGES:-}" ]; then
  echo "Installing global npm packages from OPENCLAW_HF_NPM_PACKAGES..."
  read -r -a HC_NPM_PACKAGES <<< "$OPENCLAW_HF_NPM_PACKAGES"
  if npm install -g "${HC_NPM_PACKAGES[@]}"; then
    echo "OPENCLAW_HF_NPM_PACKAGES install complete."
  else
    HC_STARTUP_FAILURES=$((HC_STARTUP_FAILURES + 1))
    echo "ERROR: OPENCLAW_HF_NPM_PACKAGES install failed: ${OPENCLAW_HF_NPM_PACKAGES}" >&2
  fi
fi
if [ -n "${OPENCLAW_HF_PLUGINS:-}" ]; then
  echo "Installing OpenClaw plugins from OPENCLAW_HF_PLUGINS..."
  read -r -a HC_OPENCLAW_PLUGINS <<< "$OPENCLAW_HF_PLUGINS"
  if openclaw plugins install "${HC_OPENCLAW_PLUGINS[@]}"; then
    echo "OPENCLAW_HF_PLUGINS install complete."
  else
    HC_STARTUP_FAILURES=$((HC_STARTUP_FAILURES + 1))
    echo "ERROR: OPENCLAW_HF_PLUGINS install failed: ${OPENCLAW_HF_PLUGINS}" >&2
  fi
fi
...
# Recommended: use one variable, OPENCLAW_HF_RUN, as a full bash script. If the
# value starts with base64: or b64:, the rest is decoded and run as the script.
# Legacy granular OPENCLAW_HF_STARTUP_* variables are still supported below.
if [ -n "${OPENCLAW_HF_RUN:-}" ]; then
  hc_run_startup_auto "OPENCLAW_HF_RUN" "$OPENCLAW_HF_RUN" || true
fi
if [ -n "${OPENCLAW_HF_STARTUP_COMMANDS:-}" ]; then
  echo "Running commands from OPENCLAW_HF_STARTUP_COMMANDS..."
  hc_run_command_block "OPENCLAW_HF_STARTUP_COMMANDS" "$OPENCLAW_HF_STARTUP_COMMANDS"
fi
for HC_STARTUP_INDEX in $(seq 1 100); do
  HC_STARTUP_VAR="OPENCLAW_HF_STARTUP_COMMAND_${HC_STARTUP_INDEX}"
  if [ -n "${!HC_STARTUP_VAR:-}" ]; then
    hc_run_startup_command "$HC_STARTUP_VAR" "${!HC_STARTUP_VAR}" || true
  fi
done
if [ -n "${OPENCLAW_HF_STARTUP_SCRIPT:-}" ]; then
  hc_run_startup_script "OPENCLAW_HF_STARTUP_SCRIPT" "$OPENCLAW_HF_STARTUP_SCRIPT" || true
fi
if [ -n "${OPENCLAW_HF_STARTUP_SCRIPT_B64:-}" ]; then
  hc_run_startup_script_b64 "OPENCLAW_HF_STARTUP_SCRIPT_B64" "$OPENCLAW_HF_STARTUP_SCRIPT_B64" || true
fi
for HC_STARTUP_INDEX in $(seq 1 20); do
  HC_STARTUP_VAR="OPENCLAW_HF_STARTUP_SCRIPT_B64_${HC_STARTUP_INDEX}"
  if [ -n "${!HC_STARTUP_VAR:-}" ]; then
...
start_background_devdata_sync() {
  ...
  if [ "${DEVDATA_DATASET_NAME:-openclaw-hf-devdata}" = "${BACKUP_DATASET_NAME:-openclaw-hf-backup}" ]; then
