...
KEEPALIVE_STATUS_FILE = Path("/tmp/openclaw-hf-cloudflare-keepalive-status.json")
...
def slugify(value: str) -> str:
    ...
    return (cleaned or "openclaw-hf-proxy")[:63].rstrip("-")
...
async function ping(source) {{
  ...
  try {{
    const response = await fetch(TARGET_URL, {{
      method: "GET",
      headers: {{
        "user-agent": "OpenClaw Cloudflare KeepAlive",
...
def setup_keepalive_worker(api_token: str, account_id: str, subdomain: str) -> None:
    ...
    cron = os.environ.get("CLOUDFLARE_KEEPALIVE_CRON", "0 0 * * *").strip()
