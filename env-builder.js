  ...
  },
{
    "g": "Startup",
    "icon": "⚡",
    "k": "OPENCLAW_HF_STARTUP_COMMANDS",
  ...
  },
{
    "g": "Startup",
    "icon": "⚡",
    "k": "OPENCLAW_HF_STARTUP_SCRIPT_B64",
...
const safeKey = k => /^[A-Z_][A-Z0-9_]*$/.test(k) && !['OPENCLAW_HF_ENV_BUNDLE', 'ENV_BUNDLE'].includes(k);
...
function decodeBundle(raw) {
  try {
    ...
    if (raw.includes('OPENCLAW_HF_ENV_BUNDLE=')) {
      raw = raw.split('OPENCLAW_HF_ENV_BUNDLE=').pop().trim();
    }
...
function parseEnv(text) {
  ...
  if (
    ...
    text.includes('OPENCLAW_HF_ENV_BUNDLE=')
  ) {
...
function generateBundle() {
  ...
  $('envLineOut').value = bundle ? `OPENCLAW_HF_ENV_BUNDLE=${bundle}` : '';
}
...
try {
  ...
  console.error('OpenClaw ENV Builder init error:', e);
